...
2. 광고 단위 생성 — **배치 원칙: 본문(읽기 영역)에는 광고를 넣지 않는다.**
   ...
   | 2 | 우측 패널 (인기 태그 아래, TOC 위, 데스크톱 전용) | 디스플레이 광고 (사각형) | `adsense.slot_panel` ✅ 완료 |
...
>   일반 흐름으로 배치된다. sticky 고정은 페이지 상단에서 인기 태그를 덮는 문제가 있어 쓰지 않고,
>   sticky 인 TOC 뒤에 두면 스크롤 시 TOC 와 겹치므로 반드시 TOC 앞에 위치해야 한다.
