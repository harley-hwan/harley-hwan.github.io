...
- ✅ **AdSense**: 글 하단(post-bottom) + 우측 패널(panel, 전 페이지) + 홈 하단(home_feed,
  글 목록·페이지 번호 아래) + 페이지 하단(page_bottom, 카테고리/태그/아카이브/소개) 가동.
  본문 내부·글 카드 사이 배치는 사용 안 함 (읽기 흐름 보호).
  홈 하단/페이지 하단은 post-bottom 단위(5813890411)를 재사용 중 — 전용 단위 생성은 선택.
  남은 선택지: 모바일 앵커 광고(대시보드 자동광고에서 앵커만)
...
2. 광고 단위 생성 — **배치 원칙: 본문(읽기 영역)에는 광고를 넣지 않는다.**
   ...
   | 3 | 홈 하단 (글 목록·페이지 번호 아래) | 디스플레이 광고 (반응형) | `adsense.slot_home_feed` ✅ 가동 (post-bottom 재사용) |
