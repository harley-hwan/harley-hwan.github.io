...
- ✅ **AdSense**: 글 하단(post-bottom) + 우측 패널(panel) 가동. 본문 내부 배치는 정책상 사용 안 함.
  남은 선택지: 홈 피드(slot_home_feed), 모바일 앵커 광고(대시보드 자동광고에서 앵커만)
...
2. 광고 단위 생성 — **배치 원칙: 본문(읽기 영역)에는 광고를 넣지 않는다.**
   ...
   | 2 | 우측 패널 하단 (TOC 아래, 스크롤 시 우측 하단 고정, 데스크톱 전용) | 디스플레이 광고 (반응형/사각형) | `adsense.slot_panel` ✅ 완료 |
...
> - **우측 패널 광고(slot_panel)는 본문 컬럼 밖**(데스크톱 화면 우측의 빈 공간)에 위치하며
>   스크롤해도 우측 하단에 머문다(sticky). 본문을 가리지 않고 항상 보이는 위치라 효율이 좋다.
