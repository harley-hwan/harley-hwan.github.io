...
description: "Ubuntu 환경에서 NVIDIA GPU 드라이버와 CUDA 툴킷을 설치하고 컴파일에 필요한 도구를 준비하는 과정을 명령어 단위로 정리한다."
...
- `nvidia-driver-XXX`: NVIDIA GPU 드라이버 패키지 이름. 여기서 `XXX`는 드라이버의 버전을 나타낸다.
...
- 이 패키지에는 GCC 컴파일러와 기타 필수 도구들이 포함되어 있어, CUDA 프로그램을 컴파일하는 데 필요하다.
