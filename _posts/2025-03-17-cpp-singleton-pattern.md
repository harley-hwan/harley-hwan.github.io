...
- 인스턴스의 유일성 보장: 클래스의 객체가 반드시 하나만 존재해야 할 경우 사용된다.
- 전역 접근점 제공: 어디서든 동일한 인스턴스를 공유하여 사용 가능하다.
- 자원 관리의 효율성: 자원을 효율적으로 관리하거나 공유된 데이터 접근을 통제하기 위해 사용된다.
...
전역적으로 접근 가능하고 인스턴스의 유일성을 보장한다는 장점이 있지만, 전역 객체처럼 사용되기 때문에 의존성 관리가 어려워질 수 있다는 단점도 있다.
...
- 환경설정(Configuration): 프로그램의 설정 정보를 전역적으로 공유할 때 사용된다.
- 로깅(Logger): 로그를 기록할 때 단일 로그 관리 객체를 사용한다.
- 데이터베이스 연결: DB 연결을 관리하는 객체를 하나만 생성하여 공유할 때 사용한다.
