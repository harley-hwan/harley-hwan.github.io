...
`torch.dot()` 함수는 두 1차원 벡터 간 내적(dot product)을 계산한다. 1차원 텐서만 가능하다.
...
`torch.matmul()` 함수는 행렬 간 곱셈 연산을 수행한다. 이 연산은 1차원-2차원, 2차원-2차원 및 3차원 이상의 텐서들 간에도 가능하다.
...
배치 크기는 두 텐서가 같거나, 하나가 1이면 연산 가능하다.
