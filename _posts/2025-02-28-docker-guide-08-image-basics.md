...
이미지는 컨테이너를 실행하기 위한 파일 시스템의 특정 시점을 저장한 압축 파일이다. 쉽게 말해, 소프트웨어와 실행 환경을 포함한 패키지라고 생각하면 된다.
...
### 2. 직접 이미지를 제작
사용자는 필요에 따라 커스텀 이미지를 제작할 수도 있다. Dockerfile을 사용하여 직접 이미지를 만드는 방법은 이후 글에서 다룬다.

## 정리

이미지는 컨테이너 실행을 위한 패키지로, 운영체제와 의존성, 실행할 소프트웨어를 포함한다. 이미지를 사용하면 복잡한 설치 과정 없이 컨테이너를 실행할 수 있고, 어디서든 동일한 환경이 보장된다. 이미지는 Docker Hub에서 다운로드하거나 직접 만들 수도 있다.

다음 글에서는 이미지를 다운로드하고 공유하는 공간인 레지스트리(Docker Hub)를 다룬다.
