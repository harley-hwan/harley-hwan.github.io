...
slug: '(1)StartCuda'
categories: [Dev, CUDA]
tags: [cuda, gpu, parallel, nvidia, gpgpu, cpp, optimization, threads, memory]
---
## CUDA 프로그래밍 준비 사항
...
### 2. 소프트웨어 설치
   - CUDA Toolkit: NVIDIA의 공식 웹사이트에서 적절한 버전의 CUDA Toolkit을 다운로드 받아 설치한다.
   - GPU 드라이버: CUDA Toolkit과 호환되는 NVIDIA GPU 드라이버를 설치한다.
   - cuDNN: 딥러닝을 위한 CUDA용 딥 뉴럴 네트워크 라이브러리다. 필요한 경우 다운로드 받아 설치하면 된다.
   - IDE: NVIDIA는 Nsight라는 통합 개발 환경도 제공한다. Visual Studio와 같은 다른 IDE와 함께 CUDA 개발을 할 수도 있다.

### 3. 프로그래밍 지식
   - C/C++ 지식: CUDA는 C/C++을 기반으로 한 프로그래밍 모델이므로 기본적인 C/C++ 프로그래밍 지식이 필요하다.
   - 병렬 프로그래밍: CUDA는 GPU의 병렬 처리 능력을 최대한 활용하기 위한 프로그래밍 모델이다. 따라서 병렬 프로그래밍에 대한 이해도 중요하다.
...
장치 관리자를 열어 디스플레이 어댑터 항목을 보면 본인의 PC에 설치된 그래픽 카드를 확인할 수 있다.

<br/>

그리고 구글에 "CUDA capable GPU list" 를 검색해서 NVIDIA CUDA zone에 들어가 내 그래픽 카드가 리스트에 있다면 CUDA 프로그래밍이 가능한 것이다.
...
"Visual Studio" 를 켜고 새 프로젝트 생성(Create a new project)를 클릭하면 프로젝트 템플릿 목록이 뜨는데, 여기서 본인이 설치한 CUDA 툴킷 템플릿을 찾아 생성한다.
...
실행하면 {1,2,3,4,5} + {10,20,30,40,50} = {11,22,33,44,55} 와 같이 정상적으로 출력되는 걸 볼 수 있다.
