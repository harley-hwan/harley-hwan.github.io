...
# 4D 배치 변환: NHWC -> NCHW (TensorFlow -> PyTorch)
...
### 1. CNN 출력을 FC 레이어 입력으로 변환
```python
# CNN 출력을 Fully Connected 레이어 입력으로 변환
...
```python
# Multi-head attention을 위한 차원 변환
...
### 3. 프레임워크 간 변환
```python
# PyTorch (NCHW) <-> TensorFlow (NHWC) 변환
