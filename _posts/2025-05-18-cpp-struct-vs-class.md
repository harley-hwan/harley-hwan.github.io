...
struct는 데이터 위주로 간단하게 묶어서 쓸 때, 주로 자료 전달이나 POD(Plain Old Data) 타입을 정의할 때 적합하다. class는 데이터와 기능을 함께 묶어 객체지향적으로 설계할 때, 즉 캡슐화, 상속, 다형성을 활용할 때 쓴다.
...
struct는 데이터를 간단히 묶는 용도로 기본 접근 지정자가 public이고, class는 데이터와 기능을 함께 묶어 객체지향적으로 관리하는 용도로 기본 접근 지정자가 private이다. 둘 다 상속이 가능하므로 설계 목적에 따라 적합한 쪽을 선택해 사용하면 된다.
