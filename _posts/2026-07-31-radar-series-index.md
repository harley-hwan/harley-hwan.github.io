...
5. [(Radar) 5. 표적 추적 — 검출 목록을 트랙으로 잇기](/posts/radar-tracking/) — 찌그러진 오차 타원, 칼만과 α-β, 게이팅과 연관, IMM, 트랙 관리, NIS 검정
6. [(Radar) 6. 위상배열 레이다 — 관성 없는 빔이 바꾸는 것](/posts/phased-array-radar/) — 배열 인자와 조향, 격자엽, 스캔 손실, 빔 스퀸트, AESA와 디지털 배열, 자원 관리

0~3장은 24 GHz 근거리 CW 를 기준으로 쓰고, 4~6장은 X 밴드 감시 레이다로 기준을 옮긴다. 파형이 바뀌면 숫자 감각이 통째로 달라지기 때문이다. 0~4장이 전파를 검출 목록으로 바꾸는 과정이고, 5장이 그 목록을 트랙으로 바꾸는 과정이라면, 6장은 그 트랙이 다시 다음 빔을 어디로 보낼지 정하는 과정이다. 고리가 6장에서 닫힌다.

### 함께 보면 좋은 글

- [(C++) 3채널 레이다 센서로 물체 운동의 스핀축 계산하기](/posts/cpp-3-channel-radar-sensor-spin-axis-calculation/) — 3장 각도 계산의 응용. RX를 'ㄴ' 자로 놓고 상하·좌우 위상차에서 스핀축을 구한다
...
- **안테나 소자 설계.** 6장은 배열 수준까지다. 개별 방사 소자의 형상과 정합, 소자 간 상호 결합(mutual coupling)은 범위 밖이다.
- **RF 프런트엔드 회로.** 믹서와 LO는 블록으로만 다루고 LNA 설계, 정합, 기판 손실, 송신관은 범위 밖이다.
- **다중 센서 융합.** 5장은 레이다 한 대까지다. 센서 정합(registration)과 순서가 뒤바뀐 측정은 이름만 짚고 넘어간다.
