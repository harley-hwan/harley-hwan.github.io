...
| 표적이 정면에서 | 세기가 오르내리는 폭 (편도) | 같은 안테나로 송수신하면 |
| ---: | ---: | ---: |
| 정면 | 없다 | 없다 |
| 빔폭의 0.05 배 | 0.97 dB | 1.94 dB |
| 빔폭의 0.10 배 | 1.95 dB | 3.90 dB |
| 빔폭의 0.20 배 | 3.94 dB | 7.89 dB |

같은 급전을 돌리면 송신과 수신이 함께 흔들리므로 왕복으로 dB 가 두 배가 된다. 오른쪽 열이 고전적인 원추 주사에서 실제로 관측되는 값이고, 수신만 돌리는 구성(COSRO)에서는 왼쪽 열이 맞다.

벗어난 만큼 폭이 커지고, 정면에 있으면 아예 오르내리지 않는다. 그러니 오르내림이 사라지도록 빔을 옮기면 표적이 저절로 한가운데 놓인다. 방위와 고각을 한꺼번에 얻는다는 것이 순차 로빙보다 나은 점이다. 대신 한 바퀴를 다 돌아야 각도가 하나 나오므로 펄스가 수십 개 들고, 빔을 비껴 놓은 만큼 정면 이득이 편도 1.9 dB, 왕복으로는 3.8 dB 깎인다.
...
**같은 개구에서는 두 방식의 이론 정확도가 사실상 같다.** 위상 비교 쪽을 셀 때 빠뜨리기 쉬운 것이 둘 있어서 그렇다. 개구를 반으로 나누면 각 반쪽 채널의 SNR 이 전체의 절반이고, 위상차는 서로 독립인 두 위상 측정의 차라 분산이 다시 두 배가 된다. 둘을 넣으면 $\sigma_{\Delta\phi} = \sqrt{2/\mathrm{SNR}}$ 이고, 여기에 $\sigma_\theta = \sigma_{\Delta\phi}\,\lambda/(2\pi D\cos\theta)$ 를 적용한 값이 위 표다. 이 정도가 한계라는 것은 개구 크기만으로 정해지는 하한과도 맞는다. 25 $\lambda$ 개구의 각도 추정 하한이 SNR 13 dB 에서 0.200° 이므로, 어느 방식이든 그보다 좋아질 수는 없다.

그러면 두 방식을 가르는 것은 정확도가 아니다. **모호성이 있느냐, 회로가 어떻게 생겼느냐, 그리고 기선을 개구 밖으로 늘릴 수 있느냐**다. 진폭 비교는 모호성이 없고 하나의 개구로 끝난다. 위상 비교는 두 개구를 물리적으로 벌릴 수 있어서, 기선을 개구 크기보다 크게 잡는 간섭계 배치에서 비로소 정확도로 앞선다. 대신 그만큼 모호성 구간이 좁아진다.
...
이 다섯 배는 두 산란점의 진폭비가 정한다. 이탈의 최대값이 $\frac{\Delta\theta}{2}\cdot\frac{1+a}{1-a}$ 이므로 진폭이 비슷할수록($a \to 1$) 발산한다. 위 값은 $a \approx 0.83$ 을 넣은 것이다.
...
이 표는 안테나를 지표에 두고 지구를 평면으로 본 값이고, 경면 반사계수 크기는 0.9 로 잡았다. 반사가 약해지면 오차도 그만큼 줄어 $\lvert\Gamma\rvert = 0.7$ 이면 첫 행이 0.17° 로 내려간다. 반대로 4/3 지구를 쓰면 50 km 에서 지표가 150 m 가까이 내려가므로 고각 열 자체가 작아진다 — 고도 200 m 표적의 실제 고각은 0.23° 가 아니라 0.06° 수준이고, 그만큼 이 표는 낙관적인 쪽이다.
