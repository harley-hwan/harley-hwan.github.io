...
- `IsProcessRunning` 함수: 이 함수는 주어진 프로세스 이름을 사용하여 프로세스가 실행 중인지 확인한다. 프로세스 이름을 비교하기 위해 `_stricmp` 함수를 사용하며, 프로세스가 실행 중이면 `true`를 반환한다.
- `main` 함수: 프로그램의 주 실행 부분
    - `IsProcessRunning` 함수를 사용하여 "DT1-Remote.exe" 프로세스가 실행 중인지 확인한다.
    - 프로세스가 실행 중이 아닐 경우, `ShellExecute` 함수를 사용하여 "DT1-Remote.exe" 프로그램을 실행한다. `system` 함수는 실행한 프로그램이 종료될 때까지 반환하지 않아서 뒤의 창을 찾는 코드에 도달하지 못하므로, 바로 반환되는 `ShellExecute`를 사용한다. 실행에 실패하면 에러 메시지를 출력하고 종료한다.
    - "DT1-Remote.exe" 프로그램이 실행되고 나서 창이 로드되는 시간을 주기 위해 3초 동안 대기한다.
    - `FindWindow` 함수를 사용하여 "DT1-Remote" 창을 찾는다. 창을 찾지 못하면 에러 메시지를 출력하고 종료한다.

위 설명은 "DT1-Remote.exe" 프로세스를 실행하고 해당 프로그램의 창을 찾는 부분까지만 다룬다. "Target on" 버튼을 찾고 작동시키는 부분은 기존 코드에 추가하면 되는데, 아래 코드에 그 부분까지 포함했다.
