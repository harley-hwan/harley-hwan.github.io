...
4. main() 함수는 다음 단계를 수행한다:
    ...
    1. 예외 처리를 수행한다. 연결 문제 등으로 인한 예외가 발생할 경우 오류 메시지를 출력한다.
...
1. Boost 라이브러리가 올바르게 설치되었는지 확인한다. Boost 라이브러리를 다운로드하고 설치하는 방법은 다음 페이지에서 확인할 수 있다: https://www.boost.org/users/download/
2. 프로젝트 설정을 열고 헤더 파일 검색 경로를 확인한다. Boost 라이브러리 헤더 파일이 있는 디렉토리를 포함하도록 경로를 업데이트해야 한다. Visual Studio를 사용하는 경우 다음 단계를 따른다:
	1. 솔루션 탐색기에서 프로젝트를 마우스 오른쪽 버튼으로 클릭하고, '속성'을 선택한다.
	1. 구성 속성 -> C/C++ -> 일반으로 이동한다.
	1. '추가 포함 디렉터리' 항목을 찾고, Boost 라이브러리 헤더 파일이 있는 디렉토리를 추가한다. 예를 들어, Boost 라이브러리가 C:\boost_1_77_0 에 설치되어 있다면, 이 디렉토리를 추가 포함 디렉터리에 추가한다.
	1. 변경 사항을 저장하고 프로젝트를 다시 빌드한다.
