...
Visual Studio 2019 Professional 또는 최신 버전에서 WPF 애플리케이션 프로젝트를 생성하는 방법을 설명한다.

1. Visual Studio 실행 후 `새 프로젝트 만들기` 선택
2. WPF 애플리케이션을 검색하여 선택 (헷갈리지 않도록 주의)
...
최신 프로젝트에서는 `.NET Core` 또는 `.NET 5/6/7`을 선택하는 것이 바람직하다. `.NET Framework`는 Windows 환경에서만 실행되며, 최신 기능이 제한될 수 있다.
...
이렇게 UI와 로직을 분리하면 코드의 재사용성이 높아지고 유지보수가 쉬워진다.
...
이렇게 MVVM 패턴을 적용해 프로젝트를 시작하면 이후 유지보수와 확장이 한결 수월해진다.
