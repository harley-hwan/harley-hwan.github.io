...
도구를 쓸 때 중요한 것은 **결과가 아니라 입력**이다. 3장에서 본 대로 표적 모델 하나가 8 dB 를 움직이고 빠진 손실 항목이 3 dB 를 움직인다. 같은 도구에 같은 레이다를 넣어도 그 두 줄을 다르게 잡으면 답이 두 배 차이 난다. **도구는 답을 주는 것이 아니라 가정을 일관되게 적용해 줄 뿐**이고, 그래서 2·3장의 내용을 알고 써야 출력을 믿을지 말지 판단할 수 있다.
...
$d$ 는 모서리 변의 길이다. 두 식은 물리광학 근사로 얻은 보어사이트 최대값이라 $d$ 가 파장보다 충분히 클 때만 쓸 수 있고, 대략 $d \gtrsim 3\lambda$ 를 기준으로 잡는다. 5.6 GHz 에서 계산해 보면 차이가 극단적이다.

| 크기 ($d/\lambda$) | 금속구 | 삼각 삼면코너 | 사각 삼면코너 |
| ---: | ---: | ---: | ---: |
| 0.15 m (2.8) | −17.5 dBsm | −1.3 dBsm | +8.2 dBsm |
| 0.30 m (5.6) | −11.5 dBsm | +10.7 dBsm | +20.3 dBsm |
| 0.60 m (11.2) | −5.5 dBsm | +22.8 dBsm | +32.3 dBsm |

첫 행은 $d = 2.8\lambda$ 라 위 근사의 유효 범위 아래에 있고, 구 쪽도 $ka = 8.8$ 로 앞 표의 광학 영역 기준($ka > 10$)에 못 미친다. 참고값으로 읽어야 한다.
