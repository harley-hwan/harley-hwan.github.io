...
[3장](/posts/cw-receive-chain-cpp/)과 [4장](/posts/pw-radar/)은 둘 다 CPI 하나에서 끝났다. 프레임 하나를 처리하면 나오는 것은 **그 순간의 검출 목록**이고, 목록의 각 줄에는 거리·각도·도플러와 SNR 이 적혀 있다.
...
기준 시스템은 4장의 X 밴드 감시 레이다를 그대로 쓴다. 여기에 갱신 주기가 추가된다.

| 항목 | 값 | 출처 |
| :--- | ---: | :--- |
| 거리 분해능 $\Delta R$ | 14.99 m | 4장 (압축 후) |
| 빔폭 $\theta_{3\mathrm{dB}}$ | 2° | 4장 |
| 검출 SNR | 13.2 dB | 4장 ($P_d$ 0.9) |
| 스캔 주기 $T$ | 4 s | 회전 감시 |
| 표적 | 250 m/s 항공기 | — |
...
- **$P_d < 1$** — 표적이 있어도 못 볼 수 있다. 4장 기준으로 $P_d = 0.9$ 이면 스캔 열 번에 한 번은 빈손이다.
...
각도 쪽은 4장 모노펄스에서 나온 식 그대로다. 기준 시스템에 넣으면
...
2장의 주파수 추정 CRLB 를 이 시스템에 적용하면 CPI 128 ms 에서 $\sigma_{\dot R}$ 이 이론적으로 10.5 mm/s 다. 실무에서 열 배 나쁘게 잡아도 **0.1 m/s** 수준이다. 반면 위치를 미분해 속도를 얻으면
...
**모호한 측정.** 4장에서 본 접힌 거리와 접힌 속도다. 겉보기 값만으로는 몇 바퀴 감겼는지 알 수 없지만, **트랙이 있으면 예측값이 그 답을 준다.** 기준 시스템에서 300 m/s 표적은 4초 동안 1.2 km 를 가는데 $R_{\max}$ 가 149.9 km 이므로 바퀴 수가 스캔 사이에 튀지 않는다. 즉 초기 획득에서 한 번만 풀면 그다음은 트랙이 유지해 준다. 초기 획득은 다중 PRF 로 푼다.
...
**갱신 주기가 일정하지 않다.** 위상배열은 표적마다 다른 주기로 조사한다. $F$ 와 $Q$ 를 $T$ 의 함수로 매번 다시 만들어야 하고, 앞의 α-β 상수는 쓸 수 없다. 그 주기를 무엇으로 정하는지 — 여기서 계산한 공분산이 그대로 그 판단 기준이 된다 — 는 [6장](/posts/phased-array-radar/)에서 다룬다.

### 짧은 궤적은 필터가 아니라 일괄 적합이 낫다

3장에서 남겨 둔 문제 — STFT 로 얻은 추정치를 궤적으로 묶고 임팩트 시점으로 역외삽하는 일 — 는 여기까지의 이야기와 성격이 다르다.
...
그리고 **추적 필터에 고정소수점은 권하지 않는다.** 공분산의 동적 범위가 초기화 직후와 수렴 후에 여러 자릿수 차이가 나서 스케일을 잡기가 어렵다. 4장의 신호처리와 달리 여기는 연산량이 작으므로 double 을 쓰는 편이 낫다.
...
트랙이 다시 다음 빔을 어디로 보낼지 정하는 쪽은 [6장](/posts/phased-array-radar/)에 있다. 여기서 계산한 공분산이 거기서 갱신 주기의 기준이 되고, 그 순간 앞의 α-β 상수는 쓸 수 없게 된다. 전체 목차는 [여기](/posts/radar-series-index/).
