...
**갱신 주기가 일정하지 않다.** 위상배열은 표적마다 다른 주기로 조사한다. $F$ 와 $Q$ 를 $T$ 의 함수로 매번 다시 만들어야 하고, 앞의 α-β 상수는 쓸 수 없다. 그 주기를 무엇으로 정하는지 — 여기서 계산한 공분산이 그대로 그 판단 기준이 된다 — 는 [10장](/posts/phased-array-radar/)에서 다룬다.
...
트랙이 다시 다음 빔을 어디로 보낼지 정하는 쪽은 [10장](/posts/phased-array-radar/)에 있다. 여기서 계산한 공분산이 거기서 갱신 주기의 기준이 되고, 그 순간 앞의 α-β 상수는 쓸 수 없게 된다.
