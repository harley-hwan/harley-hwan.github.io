...
앞의 211.3 dB 는 송신 전력과 안테나 이득, 표적 크기, 잡음 바닥을 전부 곱해 dB 로 펼친 상수항이다. $\log_{10}1 = 0$ 이므로 **거리 1 m 에서의 SNR** 이라고 읽으면 된다. 뒤의 항은 전파가 왕복하며 퍼져 나가는 몫이고, 거리가 10배 될 때마다 40 dB 씩 깎는다. **검출에 SNR 13 dB 가 필요하다고 두면** 그 값까지 떨어지는 거리가 답이다.
...
실무에서 더 자주 만나는 것은 둘째 줄이다. **LNA 앞에 있는 손실은 그대로 잡음지수에 더해진다.** 레이돔 0.3 dB, 회전 결합기 0.5 dB, 마스트까지 올라가는 급전선 1 dB 는 "신호가 1.8 dB 약해지는" 문제가 아니라 "잡음지수가 1.8 dB 나빠지는" 문제이고, 탐지거리로는 9.8 % 다. 1장 8절에서 "케이블 1 dB 가 탐지거리 5.6 % 를 깎는다" 고 적은 값의 근거가 여기 있다.
...
**문턱은 $P_{fa}$ 의 로그에만 비례한다.** $P_{fa}$ 를 $10^{-6}$ 에서 $2.3\times10^{-12}$ 로 40 만 배 조여도 문턱은 11.40 dB 에서 14.28 dB 로 2.9 dB 만 올라간다. 오경보는 값싸게 줄일 수 있다.
