...
한 칸 안에서 가장 나쁜 방향이 4.2 dB 올라온다. **수신 채널을 다섯 배로 늘려 얻은 것이 이 4.2 dB 다.** 한 빔만 쓸 때는 그 최악점이 칸의 구석이지만, 다섯 빔을 세우면 구석은 모서리 빔이 맡으므로 최악점이 이웃한 두 모서리 빔 사이의 변 가운데로 옮겨 간다. 그리고 네 모서리 빔은 각도를 재는 데도 그대로 쓰인다.
...
단 11.5 dB 는 14 회를 코히어런트로 묶었을 때의 값이다. 조사 사이의 간격이 길거나 표적의 시선 가속 때문에 위상을 유지할 수 없어 비코히어런트로 쌓으면 이득이 9 dB 남짓으로 줄고, 그러면 확장 송신 쪽이 2~3 dB 불리해진다. 어느 쪽인지가 이 비교의 전제다.
...
$(x_n, y_n)$ 은 소자 $n$ 의 위치를 파장 단위로 적은 것이고, $s_n$ 은 그 소자가 받은 신호, $y$ 는 빔 출력이다. $a_n$ 은 부엽을 낮추는 크기 가중이고 지수부가 방향을 정한다. **가중 한 벌이 빔 하나다.** 같은 수신 데이터에 가중을 $M$ 벌 곱하면 빔이 $M$ 개 나온다. 소자가 받은 숫자가 그대로 남아 있으니 몇 번을 다시 합치든 상관없다.

### 언제 위상으로 대신해도 되는가

위상은 한 주파수에서만 정확한 지연이다. 신호에 대역폭이 있으면 밴드 가장자리에서 빔이 밀린다. 대역 $\Delta f$ 의 가장자리, 곧 중심에서 $\Delta f/2$ 떨어진 곳에서 밀리는 양이 $u_0 \cdot (\Delta f/2) / f_0$ 이고, 이것이 빔폭에 견줘 작아야 협대역 근사가 성립한다.
...
마지막 줄은 공짜가 아니다. 부배열로 묶으면 그 단에서 본 소자 간격이 부배열 간격이 되므로 격자엽 조건이 되살아나고, 아날로그 위상 이동기가 가리키는 방향에서 디지털 빔을 멀리 떼어 놓을수록 그 로브가 올라온다. 이 글의 격자처럼 $u$ 로 $\pm 0.5$ 를 덮어야 하면 부배열을 겹치게 구성하거나 크기를 줄여야 하고, 그만큼 절감폭이 준다.
...
**이웃한 빔을 빼는 방식.** 격자에 이미 빔이 여러 개 서 있으므로, 마주 보는 두 빔의 차가 곧 차 빔이다. 가중을 더 만들 필요가 없다. 대신 기울기가 빔 간격에 묶인다.

네 모서리 빔이 있으면 사분면 구성을 그대로 쓰는 것이 표준이다. 왼쪽 위·오른쪽 위·왼쪽 아래·오른쪽 아래를 $B_1 \sim B_4$ 라 하면 $\Sigma = B_1+B_2+B_3+B_4$, $\Delta_{az} = (B_1+B_3)-(B_2+B_4)$, $\Delta_{el} = (B_1+B_2)-(B_3+B_4)$ 다. 빔 두 개만 빼는 간이 구성도 되기는 하는데, 받은 에너지의 절반을 버리는 데다 표적이 반대쪽 사분면에 있으면 그 두 빔의 이득이 낮아 비의 SNR 이 급격히 떨어진다.
...
16 × 16 배열에 해밍 가중을 걸고 $(u, v) = (0.021, -0.013)$ 에서 오는 평면파를 넣어 보면, 가장 센 빔은 가운데 빔이 잡히고 되짚은 값이 $(0.02115, -0.01303)$ 이 나온다. 오차가 0.00015 다. 여기서는 검증만 하려고 해밍을 썼으므로 $\Delta u_{3\mathrm{dB}}$ 와 $k_m$ 도 해밍 값으로 넣어야 하고, 앞 절 표의 Taylor $-30$ dB 계수를 그대로 가져오면 각도가 그만큼 어긋난다.
