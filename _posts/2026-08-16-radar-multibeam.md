...
마지막 줄은 공짜가 아니다. 부배열로 묶으면 디지털 쪽에서는 소자가 아니라 부배열이 늘어선 성긴 배열이 되므로 격자엽 조건이 되살아나고, 아날로그로 조향해 둔 방향에서 디지털 빔을 멀리 떼어 놓을수록 그 로브가 올라온다. 이 글의 격자처럼 넓은 범위를 덮어야 하면 부배열을 겹치게 구성하거나 크기를 줄여야 하고, 그만큼 절감폭이 준다.
