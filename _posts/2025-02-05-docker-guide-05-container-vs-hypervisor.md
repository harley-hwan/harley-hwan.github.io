...
### 컨테이너 가상화
컨테이너는 리눅스 커널이 제공하는 격리 기능을 기반으로, 운영체제(OS) 수준에서 가상화된 독립 환경을 제공한다. LXC(Linux Containers)는 이 커널 기능을 활용하는 유저스페이스 컨테이너 도구이고, 초기 Docker도 LXC를 사용했지만 2014년 이후로는 자체 구현한 libcontainer(현재의 runc)를 사용한다.
...
## 결론
컨테이너 가상화는 빠른 속도, 낮은 오버헤드, 높은 유연성 덕분에 현대 애플리케이션 환경에서 널리 쓰이는 기술이 되었다. 다만 보안이 중요한 환경에서는 하이퍼바이저 가상화가 여전히 유용하며, 두 방식을 혼합해 사용하는 경우도 있으니 환경에 맞게 선택하면 된다.
