---
title: "(Radar) 6-1. CW 수신 체인 구현 — IQ 신호에서 속도와 각도 구하기"
description: "CW 수신 체인 여섯 단계를 그림과 코드로 확인한다. I–Q 궤적으로 결함을 구분하는 법, 불균형이 반대쪽 주파수에 이미지를 만드는 이유, 창이 필요한 이유, 세 점으로 피크 위치를 보간하는 법, 위상차를 켤레곱으로 구해야 하는 이유를 다룬다."
...
[6장](/posts/cw-fmcw-pw-radar/)에서 CW는 접지 않은 1차원 도플러 스펙트럼 하나가 전부라고 정리했다. 그 스펙트럼을 실제로 뽑는 수신 체인은 여섯 단계다. DC 제거, I/Q 보정, 윈도잉, 복소 FFT, 검출, 각도. 각 단계마다 왜 필요한지가 있는데, 그걸 모르고 짜면 코드는 돌아가지만 결과가 이상해진다.
...
$\Delta v = \lambda/2T$는 창을 씌우지 않았을 때의 값이다. 이 체인은 전 구간에서 Hann을 쓰므로 실제로 두 표적을 갈라 보려면 그보다 넓어야 한다 — Hann의 −3 dB 폭이 1.44 빈이라 0.88 m/s이고, 메인로브 반폭까지 보면 1.2 m/s 정도다.
...
표적이 빈 사이에 놓여 피크가 낮아지는 것은 스캘롭 손실이라 하는데, 이쪽은 Hann이 치르는 값이 아니라 **버는 값**이다. 직사각 창은 빈 정중앙에서 3.92 dB가 깎이는데 Hann은 1.42 dB에 그친다. 메인로브가 넓어서 빈 사이가 덜 꺼지기 때문이다. 진폭을 절대값으로 써야 한다면 이 1.42 dB를 보정해야 하고, 속도만 필요하면 무시해도 된다.
...
Jacobsen 추정기는 직사각 창을 전제로 유도된 식이라 Hann에 그대로 쓰면 포물선보다 16배 나쁘다. 그런데 그 오차는 무작위가 아니라 기울기 정확히 0.5인 선형 편향이라, **결과에 2를 곱하기만 하면 편향이 사라진다.** Hann 전용으로 유도된 Grandke 추정기도 마찬가지다. 인접 빈과 피크의 선형 진폭비를 $\alpha$라 할 때 $\delta = (2\alpha-1)/(1+\alpha)$ 한 줄이면 된다. 창을 바꾸면 보간식도 다시 확인해야 한다는 것이 요점이고, 확인하고 나면 대개 고칠 수 있다.

0.016 bin을 속도로 바꾸면 9.8 mm/s다. 이 값은 잡음과 무관하게 남는다는 것이 중요하다. **정밀도의 한계가 잡음이 아니라 보간 편향이라는 뜻이고, SNR을 30 dB에서 60 dB로 올려도 이 항은 그대로다.** 위 표의 아래 두 줄이 그 편향을 없애는 방법이다.
...
> **이 검출기는 참조 셀을 dB로 평균한다.** 선형 전력으로 평균하는 교과서의 셀 평균(CA) CFAR와는 다른 물건이고, log-CFAR 계열에 속한다. 레일리 잡음에서 dB 평균 바닥은 선형 평균보다 $10\gamma/\ln 10 \approx 2.5$ dB 낮게 잡히므로($\gamma$는 오일러 상수), [3장](/posts/radar-range-degradation/) 2절의 문턱 계수 $\alpha = N_{ref}(P_{fa}^{-1/N_{ref}}-1)$는 선형 평균 전제라 `threshold_db`에 그대로 넣으면 $P_{fa}$가 설계값보다 커진다. 창을 씌우면 인접 빈의 잡음이 상관되어 유효 독립 참조 셀 수가 $N_{ref}$보다 작아지는데, 이 효과도 같은 방향으로 겹친다. 같은 이유로 이 코드가 보고하는 SNR도 잡음 전력 대비보다 그만큼 높게 읽힌다. $P_{fa}$를 사양으로 보증해야 한다면 참조 평균은 선형 전력으로 하고 dB는 표시에만 쓰는 것이 맞다.
{: .prompt-warning }
...
`arg(a) - arg(b)`로 빼면 안 된다. 두 위상은 각각 $(-\pi, \pi]$ 안에서만 표현되므로, 두 채널의 절대 위상이 그 경계를 사이에 두고 갈리면 결과가 $2\pi$만큼 틀어진다. 그림 5의 경우 실제로는 20° 차이인데 뺄셈은 340°를 준다. 켤레곱을 먼저 하고 `arg`를 한 번만 취하면 곱셈 과정에서 위상이 자동으로 접힌다.
...
Hann 창을 씌운 톤에서 **피크 양옆 빈의 복소값은 정확히 180° 어긋나 있다.** 창을 씌운 스펙트럼의 커널이 반정수 위치에서 부호를 바꾸기 때문이다. 그래서 위 식은 두 항을 더하는 것이 아니라 빼는 것이 되고, 하필 피크가 빈 정중앙에 놓였을 때 결과가 정확히 0 이 된다. 신호를 모으려던 자리에서 신호가 사라지고 위상은 잡음만 남는다.

| 피크 소수 위치 | $\lvert X_k\rvert$ | $\lvert X_{k+1}\rvert$ | 두 빈의 위상차 | 더한 값 | 뺀 값 |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 0.25 | 245.8 | 175.6 | 180.0° | 140.5 | 228.3 |
| 0.50 | 217.3 | 217.3 | 180.0° | **0.0** | 217.3 |
...
inline cf bin_value(const std::vector<cf>& X, double bin_refined) {
    ...
    // Hann 창에서 이웃 빈은 180° 어긋나 있으므로 빼야 더해진다
    return X[(k + n) % n] * (1.0 - f) - X[(k + 1 + n) % n] * f;
}
```

다만 이렇게 해서 버는 것은 크지 않다. 위 표에서 0.5 일 때 217.3 이 217.3 으로 그대로이고, 0.25 에서도 245.8 이 228.3 으로 오히려 줄었다. **위상차만 필요하다면 정수 빈 값을 쓰는 편이 안전하다.**

채널마다 각자 피크를 찾아 각자의 정수 빈을 쓰면, 두 채널이 서로 다른 빈을 고르는 순간 위상차에 빈 하나만큼의 오차가 들어간다. 기준 채널에서 찾은 빈 위치를 세 채널에 공통으로 적용해야 한다.

### 캘리브레이션

채널 SNR이 $S$(선형)일 때 위상 추정의 잡음 하한은 $\sigma_\phi \approx 1/\sqrt{2S}$이고, $d = \lambda/2$면 각도로는 $\sigma_\theta \approx \sigma_\phi / (\pi\cos\theta)$다. 정면에서 SNR 30 dB면 0.41°, 50 dB면 0.041°다. 실제 장비의 각도 오차가 이 값보다 훨씬 크다면 남은 것은 잡음이 아니라 **채널 간 위상 오프셋**이고, 그 값은 온도에 따라 흘러간다. 구현은 상수 하나를 빼는 것으로 끝나지만, 그 상수를 어떻게 얻느냐가 중요하다.
...
여기서 한 가지 더. **속도는 $\lambda$에 정비례하므로 반송파를 잘못 알고 있으면 그만큼 척도가 통째로 어긋난다.** 위 코드는 $\lambda = 12.5$ mm로 반올림해 두었는데, 발진기가 정확히 24.000 GHz라면 참값은 12.4914 mm이고 67 m/s 표적에서 46 mm/s가 어긋난다. 보간 편향 9.8 mm/s보다 크다. 보간식을 손보기 전에 $f_c$를 먼저 확정해야 한다.
...
5. Hann과 포물선(dB)은 짝이다. 창을 바꾸면 보간식도 다시 확인한다. Jacobsen을 쓰려면 계수 2를 곱해야 한다
6. 위상차는 켤레곱으로 구한다. 뺄셈이 틀어질 확률은 정면에서 0이고 축에서 벗어날수록 커진다
7. 세 채널은 같은 빈에서 뽑는다. 채널마다 피크를 찾으면 오차가 들어간다. 이웃 빈을 섞을 때는 Hann의 180° 부호를 빠뜨리지 않는다
...
여기서 짠 체인은 펄스 도플러에서 거의 그대로 반복된다. 축 하나가 늘어 거리축이 붙는 것만 다르다([6장](/posts/cw-fmcw-pw-radar/)).
