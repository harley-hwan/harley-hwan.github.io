...
이 글에서는 그 여섯 단계를 그림, 식, 그리고 코드로 확인해보자.
...
복소 스펙트럼은 원형이라 인덱스를 `% n`으로 순환시킨다. 0 Hz의 왼쪽은 음의 최대 속도이고, 2절의 빈 매핑과 같은 이야기다.
...
여기까지가 한 프레임이다. 프레임을 시간축으로 이어 붙이는 문제 — STFT로 얻은 추정치를 궤적으로 묶고, 임팩트 시점으로 역외삽하는 일 — 는 [5편](/posts/radar-tracking/)에서 다룬다. 다만 거기서 보듯, 60 ms 짜리 짧은 궤적은 재귀 필터보다 전체 구간을 한꺼번에 맞추는 일괄 적합 쪽이 맞다. 필터가 답인 경우와 아닌 경우를 5편 9절에서 갈라 두었다.

[4편](/posts/pw-radar/)은 방향을 바꿔 펄스 파형으로 넘어간다. 이 글에서 짠 체인 — DC 제거, 창, FFT, 피크 보간, 채널 위상차 — 이 펄스 도플러에서 거의 그대로 반복되는데, 축 하나가 늘어 거리축이 붙는다는 점만 다르다. 시리즈 전체 지도는 [목차](/posts/radar-series-index/)에 있다.
