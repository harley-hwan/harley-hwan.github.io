...
[6장](/posts/cw-fmcw-pw-radar/)에서 CW 수신 체인을 여섯 단계로 정리했다. DC 제거, I/Q 보정, 윈도잉, 복소 FFT, 검출, 각도. 각 단계마다 왜 필요한지가 있는데, 그걸 모르고 짜면 코드는 돌아가지만 결과가 이상해진다.
...
> **이 검출기는 참조 셀을 dB로 평균한다.** 선형 전력으로 평균하는 교과서의 셀 평균(CA) CFAR와는 다른 물건이고, log-CFAR 계열에 속한다. 레일리 잡음에서 dB 평균 바닥은 선형 평균보다 $10\gamma/\ln 10 \approx 2.5$ dB 낮게 잡히므로($\gamma$는 오일러 상수), [6장](/posts/cw-fmcw-pw-radar/)의 문턱 계수 $\alpha = N_{ref}(P_{fa}^{-1/N_{ref}}-1)$는 선형 평균 전제라 `threshold_db`에 그대로 넣으면 $P_{fa}$가 설계값보다 커진다. 같은 이유로 이 코드가 보고하는 SNR도 잡음 전력 대비보다 그만큼 높게 읽힌다. $P_{fa}$를 사양으로 보증해야 한다면 참조 평균은 선형 전력으로 하고 dB는 표시에만 쓰는 것이 맞다.
{: .prompt-warning }
